    ...
    def load_from_csv(self):
        if not os.path.exists(self.filename):
            with open(self.filename, "w") as f:
                f.write(",".join(consts.VIDEO_COLS) + "\n")
                f.close()
        self.df = pd.read_csv(self.filename, usecols=consts.VIDEO_COLS, sep=",", lineterminator="\n")
            ...
            for v in new_videos:
                ...
                try:
                    ...
                    video_data.append([v, title, publishedAt, channelId,
                                       channelName, views, likes, num_comments])
                except IndexError:
                    print("Index Error")
                    continue
            
            self.df = pd.concat([self.df, pd.DataFrame(video_data, columns=consts.VIDEO_COLS)])
