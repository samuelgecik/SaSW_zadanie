...
def get_videos_from_channel(channel_id, n):
    ...
    request = youtube.playlistItems().list(
        part="snippet, contentDetails", maxResults=n, playlistId=uploads
    )
    response = request.execute()
    video_id_list = []
...
def get_comments_from_video(videoId, n):
    ...
    if os.path.exists(consts.COMMENTS_CSV):
        df = pd.read_csv(
            consts.COMMENTS_CSV, usecols=consts.COMMENT_COLS, lineterminator="\n"
        )
        df = pd.concat([df, pd.DataFrame(comments, columns=consts.COMMENT_COLS)])
